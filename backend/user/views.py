        ...
        if to_user != from_user:
            if to_user not in from_user.friends.all():  # if not friend
                if not FriendRequest.objects.filter(to_user=to_user, from_user=from_user).exists():  # if not sent
                    if not FriendRequest.objects.filter(to_user=from_user,
                                                        from_user=to_user).exists():  # if not recieved friend request from the other user
                        friend_request = FriendRequest.objects.create(to_user=to_user,
                                                                      from_user=from_user)  # Notification 자동적으로 생성됨
                        serializer = FriendRequestDetailSerializer(friend_request)
                        return Response(serializer.data, status=status.HTTP_200_OK)
                    return Response({"detail": "there is a friend request sent from the other user"},
                                    status=status.HTTP_403_FORBIDDEN)
                return Response({"detail": "this friend request is already sent"},
    ...
    def post(self, request, name):
        ...
        if not friend_request.accepted:
            try:
                ...
                friend_request_notification = Notification.objects.get(type="friend_request",
