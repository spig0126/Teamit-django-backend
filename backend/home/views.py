...
class MainPageDetailAPIView(APIView):
    ...
    def get(self, request):
        result_data = {
            ...
            'my_active_teams': sorted(
                MyActiveTeamSimpleDetailSerializer(self.my_active_teams, context={'user': self.user}, many=True).data,
                key=lambda x: x['has_new_team_notifications'], reverse=True),
            'teams_with_same_activity': TeamSimpleDetailWithLikesSerializer(self.teams_with_same_activity,
                                                                            context={'user': self.user},
                                                                            many=True).data,
            'users_with_similar_interests': UserWithSameInterestDetailSerialzier(self.users_with_similar_interests,
                                                                                 context={'viewer_user': self.user},
                                                                                 many=True).data,
...
class ThirdPartyLoginView(APIView):
    ...
    def get(self, request):
        ...
        if not user_info:
            return Response({'detail': 'failed to fetch user info from login provider'},
                            status=status.HTTP_404_NOT_FOUND)

        # Generate firebase custom token with uid
        custom_token = generate_firebase_custom_token(user_info, login_type)
        if not custom_token:
            return Response({'detail': 'failed to generate firebase custom token'},
