    ...
    async def __call__(self, scope, receive, send):
        ...
        if authorization_header.startswith('Bearer '):
            ...
            await send({"type": "websocket.close", "code": 401,
                        "reason": "unauthorized: authentication token not provided or invalid"})
            return
        ...
        try:
            ...
            if user is None:
                await send({"type": "websocket.close", "code": 404,
                            "reason": "not_found: user not found with provided auth token"})
                return
            ...
            await send({"type": "websocket.close", "code": 500,
                        "reason": "user provided with the auth token is not a valid Firebase user, it has no Firebase UID"})
            return
